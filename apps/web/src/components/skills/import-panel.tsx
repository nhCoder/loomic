...
export function ImportPanel({
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      setImportState({ status: "error", message: "Please enter a valid URL" });
      return;
    }
    ...
    try {
      ...
      success(`Skill "${result.skill.name}" imported successfully`);
      await onImported();
    } catch (err) {
      const msg =
        err instanceof ApiApplicationError
          ? err.message
          : "Import failed. Check the URL and retry.";
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              Supports GitHub repository URLs and npm tarball URLs
            </p>
          </div>
        </div>
