...
**Virtual paths vs real paths**: the `ls` and `read_file` tools use virtual paths such as `/skills/...`, but the `execute` tool runs in a real shell. Python code must use real filesystem paths.

Rules:
1. **Font paths**: always use `os.environ["FONT_DIR"]`; do not hardcode paths.
2. **Output files**: always save with relative paths such as `output.png`; files are written inside the sandbox working directory.
3. **Do not** use virtual paths seen from `ls /skills/...` inside Python code.

### Font Usage

Fonts are accessed through the `$FONT_DIR` environment variable, which is already set in the sandbox:

```python
import os
font_dir = os.environ["FONT_DIR"]  # Use the environment variable; do not hardcode paths.
...
**All file paths must be relative**. Absolute paths can collide across concurrent users:

```
1. write_file path="generate.py"       <- relative path, do not use /tmp/xxx
...
**Never write files using absolute paths**:
- Wrong: `write_file path="/tmp/script.py"` — concurrent users can overwrite each other.
- Correct: `write_file path="script.py"` — each user has an isolated sandbox.
